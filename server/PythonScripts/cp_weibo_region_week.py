...
for week in weeks:
    ...
    for region in regions:
        print("开始统计 --"+week[0].isoformat()+"~"+week[1].isoformat()+"_"+region+"-- 词频")
        # 读取数据
        sql = "SELECT `word`, `weight`, `acrNum` FROM `fc_weibo` WHERE `time` BETWEEN '"+week[0].isoformat()+"' AND '"+week[1].isoformat()+"'" + " AND `region` LIKE '" + region + "'"
        cur.execute(sql)
        datas = cur.fetchall()
        # 统计词频
        results = {}
        ...
        for result in results.keys():
            cur.execute(sqlcp,(result,results[result],(date(2010,1,1)+timedelta(days=num)).isoformat(),region))
            db.commit()
# 关闭数据库连接
cur.close()
db.close()
