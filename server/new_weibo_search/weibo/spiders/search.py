...
class SearchSpider(scrapy.Spider):
    ...
    def get_region(self, region_code):
        ...
        for region in self.regions:
            # print("region=", region)
            if region_code == self.regions[region]['code']:
                return region
        return ""
    ...
    def parse_weibo(self, response):
        ...
        for sel in response.xpath("//div[@class='card-wrap']"):
            ...
            if info:
                weibo = WeiboItem()
                weibo['region'] = self.get_region(response.meta.get('province')['code'])
                ...
                #     for region in self.regions:
                #         print("region=", region)
                #         if response.meta.get('province')['code'] == self.regions[region]['code']:
                #             weibo['region'] = region
                # weibo['region'] = response.meta.get('province')['code']
                ...
                if retweet_sel and retweet_sel[0].xpath(
                    ...
                    retweet = WeiboItem()
                    retweet['region'] = self.get_region(response.meta.get('province')['code'])
                    ...
                    # if int(retweet['attitudes_count']) + int(retweet['reposts_count']) + int(retweet['comments_count']) > 500:
                    #     yield {'weibo': retweet, 'keyword': keyword}
                    weibo['retweet_id'] = retweet['id']
                print(weibo)
                yield {'weibo': weibo, 'keyword': keyword}
                # if int(weibo['attitudes_count'])+int(weibo['reposts_count'])+int(weibo['comments_count']) > 500:
                #     yield {'weibo': weibo, 'keyword': keyword}
